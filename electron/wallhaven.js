...
class Wallhaven {
    ...
    init() {
        let that = this
        // 设置图片请求的 Referer 头，解决 403 问题
        const filter = {
            urls: ['*://*.wallhaven.cc/*', '*://*.wallhaven.vip/*', '*://w.wallhaven.cc/*', '*://th.wallhaven.cc/*', '*://w.wallhaven.vip/*', '*://th.wallhaven.vip/*']
        };
        
        session.defaultSession.webRequest.onBeforeSendHeaders(filter, (details, callback) => {
            // 为图片请求添加 Referer 头
            if (details.resourceType === 'image' || 
                details.url.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
                // 根据 URL 确定 Referer
                let referer = 'https://wallhaven.vip/';
                if (details.url.includes('wallhaven.cc')) {
    ...
    getLocalData(page, size) {
        ...
        // 根据 customDomain 动态生成缩略图前缀
        let thPrefix = "https://th.wallhaven.vip/small/";
        if (this.localStorage.customDomain && this.localStorage.customDomain.trim() !== "") {
            let domain = this.localStorage.customDomain.trim().replace(/^https?:\/\//, '').replace(/\/$/, '');
            thPrefix = `https://th.${domain}/small/`;
        }
        ...
        if (start < this.localBgData.length) {
            ...
            for (let i = start; i < end; i++) {
                ...
                if (item.id.indexOf("wallhaven-") > -1) {
                    ...
                    item.src = `${thPrefix}${hChar}/${id}.jpg`
                } else {
                    // 非 wallhaven 图片，使用 file:// 协议
                    item.src = `file://${item.path}`;
                }
                // 不再使用 base64，改用 file:// 协议提高性能
    ...
    updateConfig(data) {
        ...
        that.localStorage.customDomain = data.customDomain || "";
        this.loadAutoStart()
