<template>
  <div class="mask" :class="showing === true ? '' : 'out'">
    ...
    <div class="img-view">
      <img class="img-class" :src="displaySrc" :style="{'max-height':calHeight}" referrerpolicy="no-referrer">
...
export default {
  ...
  computed: {
    ...
    displaySrc() {
      // 如果是本地图片，直接返回路径
      if (this.isLocal) {
        return this.imgInfo.realPath || this.imgInfo.path;
      }
      // 如果是在线图片，返回原始路径（通过 referrerpolicy 控制）
      return this.imgInfo.path;
    }
  },
  watch: {
    imgInfo(value) {
