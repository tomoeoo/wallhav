    ...
    <div class="setting-list">
      <div class="setting-group">
        ...
        <div class="setting-group-content">
          ...
          <div class="setting-item">
            <a class="setting-title">自定义域名</a>
            <el-tooltip placement="top" effect="light" popper-class="custom-tips api-key-tooltip">
              <template #content>自定义API请求域名地址<br/>留空则使用默认域名：wallhaven.vip</template>
              <a class="api-key-tips"><i class="fas fa-question-circle"></i></a>
            </el-tooltip>
            <div class="setting-content">
              <el-input class="custom-domain-input" v-model="customDomain" placeholder="例如：wallhaven.cc"/>
            </div>
          </div>
          <div class="setting-item">
...
export default {
  ...
  created() {
    ...
    this.customDomain = getLocalStorage("customDomain", "", "String")
  ...
  methods: {
    ...
    async saveConfig() {
      ...
      // 保存旧的自定义域名用于比较
      const previousCustomDomain = getLocalStorage("customDomain", "", "String")
      let params = {
      ...
      updateConfig(params).then(res => {
        ...
        if (res.success) {
          ...
          localStorage.setItem("customDomain", params['customDomain'])
          
          // 如果自定义域名发生变化，提示用户刷新页面
          if (previousCustomDomain !== this.customDomain) {
            this.$message({
              message: "自定义域名已更改，建议刷新页面以应用新配置",
              type: "warning",
              duration: 3000,
              customClass: 'customer-message'
            })
