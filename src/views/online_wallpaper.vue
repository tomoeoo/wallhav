  ...
  <div id="WallHaven">
    ...
    <form id="searchbar" class="expanded" @click="resetSelect">
      ...
      <div @click.stop="" id="search-category-checks" class="framed">
        <input type="checkbox" v-model="customParams.categories" name="general" value="general" id="search-general" />
        ...
        <input type="checkbox" v-model="customParams.categories" name="people" value="people" id="search-people" />
        <label for="search-people">人物</label>
      </div>
      <div @click.stop="" id="search-aiart-checks" class="framed">
        <input type="checkbox" v-model="customParams.aiArt" value=true name="ai_art_filter" id="search-ai">
      ...
      <div @click.stop="" id="search-purity-checks" class="framed">
        ...
        <input type="checkbox" v-model="customParams.purity" name="sketchy" value="sketchy" id="search-sketchy" />
      ...
      <div @click.stop="" id="search-resolutions" class="framed searchbar-dropdown">
        ...
        <div class="dropdown " :class="customParams.selector === 1 ? 'extended' : 'collapsed'">
          <div>
            <div class="framed">
              <input type="radio" v-model="customParams.respickerLimitation" name="searchbar-respicker-limitation"
                id="searchbar-respicker-atleast" value="atleast" />
              <label for="searchbar-respicker-atleast"> <i class="far fa-plus"></i> 至少 </label>
              <input type="radio" v-model="customParams.respickerLimitation" name="searchbar-respicker-limitation"
                id="searchbar-respicker-exactly" value="exactly" />
            ...
            <div class="respicker">
                  ...
                  <tr v-for="(line, i) in searchMeta.resolutionsArray">
                    <td v-for="(rln, x) in line.item">
                      ...
                      <input v-if="customParams.respickerLimitation === 'atleast'" type="radio"
                        name="respicker-resolution" :id="'searchbar-respicker-' + (rln)" :value="rln"
                        v-model="customParams.resolution" />
                      <label :for="'searchbar-respicker-' + (rln)" original-title="Dual 1080p">{{ this.$formatMulti(rln)
                      }}</label>
                    </td>
                  </tr>
                </tbody>
              ...
              <div class="respicker-custom oneline framed">
                ...
                <input type="text" pattern="[0-9]{0,5}" v-model="customParams.respickerCustomWidth"
                  name="respicker-custom-width" id="searchbar-respicker-custom-width" placeholder="Width"
                  maxlength="5" />
                ...
                <input type="text" pattern="[0-9]{0,5}" v-model="customParams.respickerCustomHeight"
                  name="respicker-custom-height" id="searchbar-respicker-custom-height" placeholder="Height"
                  maxlength="5" />
              </div>
            </div>
          </div>
                ...
                <tbody>
                  ...
                  <tr v-for="(line, i) in searchMeta.ratiosArray">
                    <td v-for="(ra, x) in line.item">
                      <input v-if="ra !== ''" v-model="customParams.ratios" type="checkbox" name="ratio" :value="ra"
                        class="ratio" :id="'searchbar-ratio-' + (ra)" />
      ...
      <div @click.stop="" id="search-initial-page" class="framed searchbar-dropdown">
        <!-- <a class="jsAnchor dropdown-toggle collapsed" style="min-width: 5em">初始页码</a> -->
        <input id="initial-page-input" type="text" v-model.number="customParams.initialPage" placeholder="页码"
          class="initial-page-input" />
      </div>
      <div @click.stop="" id="search-sorting" class="framed searchbar-dropdown">
        ...
        <div class="dropdown" :class="customParams.selector === 4 ? 'extended' : 'collapsed'">
          <div>
            ...
            <input type="radio" v-model="customParams.sorting" name="sorting" value="views" id="search-sorting-views" />
            ...
            <input type="radio" v-model="customParams.sorting" name="sorting" value="hot" id="search-sorting-hot" />
            <label for="search-sorting-hot">热度</label>
          </div>
        </div>
      </div>
      <div @click.stop="" :style="customParams.sorting !== 'toplist' ? 'visibility:hidden' : ''"
        id="search-toplist-range" class="framed searchbar-dropdown">
        ...
        <div class="dropdown" :class="customParams.selector === 5 ? 'extended' : 'collapsed'">
          <div>
            <input type="radio" v-model="customParams.topRange" name="top" value="1d" id="searchbar-toplist-range-1d" />
            <label for="searchbar-toplist-range-1d">1 天</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="3d" id="searchbar-toplist-range-3d" />
            <label for="searchbar-toplist-range-3d">3 天</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="1w" id="searchbar-toplist-range-1w" />
            <label for="searchbar-toplist-range-1w">上周</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="1M" id="searchbar-toplist-range-1M"
              checked="" />
            <label for="searchbar-toplist-range-1M">1 个月</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="3M" id="searchbar-toplist-range-3M" />
            <label for="searchbar-toplist-range-3M">3 个月</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="6M" id="searchbar-toplist-range-6M" />
            <label for="searchbar-toplist-range-6M">6 个月</label>
            <input type="radio" v-model="customParams.topRange" name="top" value="1y" id="searchbar-toplist-range-1y" />
      ...
      <div id="thumbs" class="thumbs-container thumb-listing infinite-scroll">
        <section class="thumb-listing-page" v-for="(sectionItem, i) in pageData.sections">
          <header v-if="i !== 0" class="thumb-listing-page-header">
            <h2>Page <span class="thumb-listing-page-num">{{ customParams.initialPage ? i + customParams.initialPage : i
              +
                1 }}</span> / {{ pageData.totalPage }}</h2>
...
export default {
  ...
  methods: {
    ...
    changeParams() {
      ...
      // 使用初始页码，如果未设置则从1开始
      this.pageData.currentPage = (this.customParams.initialPage && this.customParams.initialPage > 0)
        ? this.customParams.initialPage - 1
        : 0;
      this.customParams.selector = 0;
      if (this.customParams.categories.length === 0) {
